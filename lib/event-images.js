  ...
  async convert(url, key) {
    ...
    for (const size of [640, 1280]) {
      const { data, info } = await sharp(input, { animated: true, limitInputPixels: 40 * 1000 * 1000 })
        ...
        .webp({ quality: 80, effort: 3 }).toBuffer({ resolveWithObject: true });
      const file = `${key}-${size}.webp`;
  ...
  async prepare(events) {
    ...
    const worker = async () => {
      while (next < tasks.length) {
        ...
        try {
          ...
          event.images[index] = variants[variants.length - 1].url;
    ...
    if (Date.now() - this.lastCleanup > 3600000) {
      ...
      for (const name of await fs.readdir(this.directory).catch(() => [])) {
        const match = /^([a-f0-9]{32})(?:-(?:640|1280)\.webp|\.json)$/.exec(name);
